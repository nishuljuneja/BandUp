...
export default function GrammarLessonPage() {
  ...
  // Seed for shuffling ΓÇö changes each "New Questions" click
  ...
  if (!lesson) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <Link href="/grammar" className="text-indigo-600 hover:underline">ΓåÉ Back to Grammar</Link>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 sm:p-8">
              ...
              {lesson.content.examples.map((ex, i) => (
                <div key={i} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  {ex.translations[uiLanguage] && (
                    <p className="text-sm text-gray-500 mt-1">ΓåÆ {ex.translations[uiLanguage]}</p>
                  )}
                </div>
              ))}
            </div>
          ...
          >
            {t('common.practice', uiLanguage)} ({roundExercises.length} questions) ΓåÆ
          </button>
        </div>
      </div>
    );
